...
#VSRX Desktop
    def setupValkyrie(self):
        sp.call("""
            sudo cp -a valkyrie_desktop/valkyrie_tmp/etc/skel /etc
            sudo chown -R root /etc/skel
            sudo chgrp -R root /etc/skel
            sudo chmod -R 755 /etc/skel
            sudo cp -a valkyrie_desktop/valkyrie_tmp/usr/share /usr
            ...
            sudo cp -a valkyrie_desktop/valkyrie_tmp/etc/xdg /etc
    ...
    def suckout(self):
        sp.call("""
            cp -a ~/.config/openbox valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.config/fbpanel valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.config/conky valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.config/nitrogen valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.config/gtk-2.0 valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.config/gtk-3.0 valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.config/Thunar valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.config/fcitx valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.config/fontconfig valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.config/jack valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.config/plank valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.config/pulse valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.config/xfce4 valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            cp -a ~/.gtkrc-2.0 valkyrie_desktop/valkyrie_tmp/etc/skel
            cp -a ~/.Xdefaults valkyrie_desktop/valkyrie_tmp/etc/skel
            cp -a ~/.emacs.d valkyrie_desktop/valkyrie_tmp/etc/skel
            cp -a ~/.gconf valkyrie_desktop/valkyrie_tmp/etc/skel
            cp -a ~/.themes valkyrie_desktop/valkyrie_tmp/etc/skel
            cp -a /usr/share/backgrounds valkyrie_desktop/valkyrie_tmp/usr/share
            cp -a /usr/share/icons/start-here.png valkyrie_desktop/valkyrie_tmp/usr/share/icons
            cp -a /opt/openbox-logout.py valkyrie_desktop/valkyrie_tmp/opt
            cp -a /etc/apt/sources.list.d valkyrie_desktop/valkyrie_tmp/etc/apt
            cp -a /etc/apt/sources.list valkyrie_desktop/valkyrie_tmp/etc/apt
            cp -a /etc/xdg valkyrie_desktop/valkyrie_tmp/etc
            cp -a /etc/lsb-release valkyrie_desktop/valkyrie_tmp/etc/lsb-release
        """,shell=True)
        print("Finished")
    def set_plymouth(self):
        sp.call("""
            ...
            sudo cp -a valkyrie_desktop/valkyrie_tmp/lib/plymouth /lib
    ...
    def set_sources_list(self):
        sp.call("""
            sudo cp -a valkyrie_desktop/valkyrie_tmp/etc/apt/* /etc/apt
            sudo apt-get update
            sudo dpkg --set-selections < linux-set/synaptic_list
            sudo apt-get dselect-upgrade
            sudo apt-get upgrade
        """,shell=True)
    def set_wireless_poweroff(self):
        sp.call("""
            sudo cp -a valkyrie_desktop/valkyrie_tmp/etc/pm/power.d/wireless /etc/pm/power.d
    ...
    def set_blacklist(self):
        sp.call("""
            sudo cp -a valkyrie_desktop/valkyrie_tmp/etc/modprobe.d/blacklist.conf /etc/modprobe.d
...
sudo apt-get update""",shell=True)
    def installApplications(self):
        ba = ["openbox","obconf-qt","obmenu","pcmanfm-qt","lxappearance","lxrandr","lxinput","lxsession-logout",
"plank","stalonetray","dunst",
"xfce4-terminal","xfce4-taskmanager","xfce4-settings","xfce4-power-manager",
"network-manager-gnome",
"emacs25-nox","featherpad",
"simplescreenrecorder","handbrake","winff-qt",
"audacity","ffmpeg",
"pulseaudio","pavucontrol","pulseaudio-utils",
"pulseaudio-module-jack","jackd","jackd2","qjackctl",
"alsamixergui","gnome-alsamixer",
"mpv","smplayer",
"gimp","imagemagick","lximage-qt",
"chromium-browser",
"gnumeric","abiword",
"open-jtalk","mecab","mecab-utils","mecab-naist-jdic","mecab-naist-jdic-eucjp","mecab-jumandic","python-mecab",
"python-tk","python3-tk","cython","python3-sphinx",
"libllvm7","conky-all",
"libjpeg8","libjpeg8-dev","zlib1g-dev",
"libfreetype6","libfreetype6-dev",
"playonlinux",
"ppsspp","pcsx2-unstable:i386",
"apache2","libapache2-mod-php7.0","mysql-common",
"php7.0-mysql","libmysqlclient20","mysql-server-5.7","libncurses5-dev",
"fakeroot","build-essential","git",
"avidemux2.7-qt5","avidemux2.7-qt5-data","avidemux2.7-plugins-common","avidemux2.7-plugins-qt5"]
        for i in ba:
            sp.call("sudo apt-get install -y %s" % (i),shell=True)
    def installPythonModules(self):
        list2 = ["scipy","numpy","pandas","seaborn","scikit-learn","statsmodels","beautifulsoup4","matplotlib"]
        for i in list2:
            sp.call("sudo pip2 install --no-cache-dir -I %s" % (i),shell=True)
            sp.call("sudo pip3 install --no-cache-dir -I %s" % (i),shell=True)
    def installPepper(self):
        sp.call("""sudo apt-get -y install pepperflashplugin-nonfree
...
#net.ipv4.tcp_congestion_control=westwood
        ...
        f4s.close()
#jsay
    def talk(self,text,voice,mode):
        import MeCab
        if(voice == "mei_happy"):
            #modelv = homedir+"alice/file/mei/mei_happy.htsvoice"
            modelv = homedir+"alice/voice/nitech_jp_atr503_m001.htsvoice"
        if(voice == "mei_angry"):
            modelv = homedir+"alice/file/mei/mei_angry.htsvoice"
        if(voice == "mei_bashful"):
            modelv = homedir+"alice/file/mei/mei_bashful.htsvoice"
        if(voice == "mei_normal"):
            modelv = homedir+"alice/file/mei/mei_normal.htsvoice"
        if(voice == "mei_sad"):
            modelv = homedir+"alice/file/mei/mei_sad.htsvoice"
        if(voice == "miku_a"):
            modelv = homedir+"alice/file/miku/TYPE-α.htsvoice"
        if(voice == "miku_b"):
            modelv = homedir+"alice/file/miku/TYPE-β.htsvoice"
        if(mode == "data"):
...
-ow $TMP && \
aplay --quiet $TMP
rm -f $TMP""" % (modelv,text),shell=True)
        if(mode == "string"):
            sp.call("""TMP=/tmp/jsay.wav
echo %s | open_jtalk \
-x %s \
...
-ow $TMP && \
aplay --quiet $TMP
rm -f $TMP""" % (text,"/var/lib/mecab/dic/open-jtalk/naist-jdic",modelv),shell=True)
#Changed PNG to JPEG
    def png2jpg(self):
        inputdir = sys.argv[2]
...
if __name__ == "__main__":
    ...
    try:
        ...
        if argvs[1] == "ppa":
            v.addPPA()
        if argvs[1] == "jsay":
            v.talk(argvs[2],argvs[3],argvs[4])
        if(argvs[1] == "png2jpg"):
            v.png2jpg()
        if(argvs[1] == "png2gif"):
            v.png2gif()
