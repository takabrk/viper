...
#VSRX Desktop
    ...
    def suckout(self):
        sp.call("""
            ...
            cp -a ~/.config/tint2 valkyrie_desktop/valkyrie_tmp/etc/skel/.config
            ...
            rm -r valkyrie_desktop/valkyrie_tmp/usr/share/backgrounds
            cp -a /usr/share/backgrounds valkyrie_desktop/valkyrie_tmp/usr/share
            rm -r valkyrie_desktop/valkyrie_tmp/etc/apt/sources.list.d
            cp -a /etc/apt valkyrie_desktop/valkyrie_tmp/etc
            rm -r valkyrie_desktop/valkyrie_tmp/etc/xdg
            cp -a /etc/xdg valkyrie_desktop/valkyrie_tmp/etc
            cp -a /etc/lsb-release valkyrie_desktop/valkyrie_tmp/etc/lsb-release
            cp -a /etc/pulse/daemon.conf valkyrie_desktop/valkyrie_tmp/etc/pulse
            cp -a /etc/rc.local valkyrie_desktop/valkyrie_tmp/etc
        """,shell=True)
        print("Finished")
...
sudo apt-get update""",shell=True)
    def installApplications(self):
        base = ["lxqt-core","obconf-qt","obmenu","lxappearance","lxrandr","lxinput","lxsession-logout",
"dunst","tint2","gmrun",
...
"mpv","smplayer",
"gimp","imagemagick","lximage-qt","chromium-browser",
"ffmpeg","obs-studio","audacity",
...
#jsay
    def talk(self,text,voice):
        ...
        if(voice == "tohoku_angry"):
            modelv = homedir+"alice/file/tohoku/tohoku-f01-angry.htsvoice"
        if(voice == "tohoku_happy"):
            modelv = homedir+"alice/file/tohoku/tohoku-f01-happy.htsvoice"
        if(voice == "tohoku_neutral"):
            modelv = homedir+"alice/file/tohoku/tohoku-f01-neutral.htsvoice"
        if(voice == "tohoku_sad"):
            modelv = homedir+"alice/file/tohoku/tohoku-f01-sad.htsvoice"
...
class PrepareChain(object):
    ...
    DB_PATH = "alice/chain.db"
    DB_SCHEMA_PATH = "alice/schema.sql"
